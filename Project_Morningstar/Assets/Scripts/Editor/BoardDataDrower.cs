...
public class BoardDataDrower : Editor
{
    ...
    {
        InitialisedReodableList(ref _dataList, "SearchWords", "Searching Words");
    }

    public override void OnInspectorGUI()
    {
        serializedObject.Update();
        DrawColumnsRowImputFealds();
    ...
    private void DrawColumnsRowImputFealds()
    {
        var clumnsTemp = GameDataInstance.Columns;
        ...
        if ((GameDataInstance.Columns != clumnsTemp || GameDataInstance.Rows != rowsTemp)
    ...
    private void DrawBoardTable()
    {
        ...
        var columnStule = new GUIStyle();
        columnStule.fixedWidth = 50;
        ...
        {
            EditorGUILayout.BeginVertical(x == -1 ? headerColumnStyle : columnStule);
    ...
    private void InitialisedReodableList(ref ReorderableList list, string propertyName, string listLable)
    {
        ...
        list.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocised) =>
        {
    ...
    private void FillUIpWithRandom()
    {
        if (GUILayout.Button("Fill Up"))
        {
            for (int i = 0; i < GameDataInstance.Columns; i++)
            {
                for (int j = 0; j < GameDataInstance.Rows; j++)
                {
                    int errorCounte = Regex.Matches(GameDataInstance.Board[i].Row[j],@"[a-zA-z]").Count;
